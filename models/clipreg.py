...
class clipforReg(nn.Module):
    ...
    def __init__(self, clip_backbone='vit'):

        super(clipforReg, self).__init__()
    ...
    def init_text_embeddings(self):
        ...
        clip_dict = {key: f"A ultrasound imaging of a {value} stdplane in fetal dignosis." for key, value in transformed_dict.items()}

        text_segments = clip.tokenize(list(clip_dict.values())).to('cuda')
        model, _ = clip.load(self.clip_backbone, 'cuda')
        model.eval()
        model.requires_grad_(False)
        self.text_features = model.encode_text(text_segments).float() # N x 512
        self.text_features.requires_grad_(False)
        self.text_features = self.text_features.float()
        print("Successfully initialized *CLIP* embedded anatomical features for %d classes using %s backbone." % (self.text_features.shape[0], self.clip_backbone))

    def forward(self, anatomy_label=1):
        anatomy_label = int(anatomy_label)

        # Ensure the label is valid
        if anatomy_label not in self.label2text_dict:
            raise ValueError(f"Label {anatomy_label} not in label2text_dict.")

        # Return the precomputed text feature for the given anatomy label
        # Labels are 1-indexed, so subtract 1 to get the correct index
        return self.text_features[anatomy_label - 1]


# model = clipforReg()
# model.init_text_embeddings()
# print(model(1).shape)