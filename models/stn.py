...
class ResNet(nn.Module):
    def __init__(self, args, block, layers, text_dim=768, img_dim=448, embed_dim=512):
        ...
        self.cfa = CrossAttentionBlock(text_dim, img_dim, embed_dim)  # 实例化 CrossAttentionBlock
        # Define the deconvolution layer to reconstruct the image
    ...
    def forward(self, x: Tensor, Rec=True) -> Tensor:
        ...
        if Rec:
            ...
            self.train_outputs['layer1'].append(self.stn1_output)
            self.train_outputs['layer2'].append(self.stn2_output)
            self.train_outputs['layer3'].append(self.stn3_output)
            ...
            text_feature = torch.stack([text_feature] * 4, dim=0).squeeze(1)
